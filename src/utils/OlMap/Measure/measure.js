 ...
 * @LastEditors: 张仕山
 * @LastEditTime: 2022-03-30 16:15:15
 * @Description: 测距和侧面
 * @FilePath: \src\utils\OlMap\Measure\measure.js
 */
...
class Measure {
  ...
  createHelpTooltip() {
    ...
    this.olmap.map.addOverlay(this.helpTooltip);
  }
  ...
  createMeasureTooltip() {
    ...
    this.olmap.map.addOverlay(this.measureTooltip);
  }

  // 计算长度
  formatLength(line) {
    const sourceProj = this.olmap.map.getView().getProjection(),
  ...
  addInteraction(value) {
    this.count += 1;
    const pointerMoveHandler = this.olmap.map.on(
      "pointermove",
      this.pointerMoveHandler
    );

    this.olmap.map.getViewport().addEventListener("mouseout", () => {
      this.helpTooltipElement.classList.add("hidden");
    });

    let type = value === "area" ? "Polygon" : "LineString",
    ...
    popupcloser.onclick = () => {
      ...
      const count = popupcloser.getAttribute("data-count"),
        overLays = this.olmap.map.getOverlays().getArray(),
        layers = this.olmap.map.getLayers().getArray();

      overLays.forEach((item) => {
        if (item.id === Number(count)) {
          this.olmap.map.removeOverlay(item);
        }
      });
  ...
  removeInteraction() {
    if (this.draw) {
      ...
      this.helpTooltipElement.classList.add("hidden");
      this.olmap.map.removeOverlay(this.helpTooltip);
    }
  }
