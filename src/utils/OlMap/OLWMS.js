 ...
 * @LastEditors: 张仕山
 * @LastEditTime: 2022-03-30 16:16:14
...
class OLWMS {
  ...
  getMosaicDataset(params, url) {
    const { cycle, time, colorArr, mark, click, max } = params;
  ...
  addOverlay(element, layerName, tpl, callback) {
    ...
    this.map.on("click", (e) => {
      const view = this.map.getView(),
        ...
        layerList = this.map
          .getLayers()
          .array_.filter((item) => item.get("name").indexOf(layerName) > -1);
      ...
      if (name && name.indexOf(layerName) > -1) {
        ...
        const { projection = "EPSG:4326", geoserverWorkspace = "ja" } = tpl,
          url1 = currentLayer
            .getSource()
            .getFeatureInfoUrl(e.coordinate, viewResolution, projection, {
              INFO_FORMAT: "application/json",
              QUERY_LAYERS: name.startsWith(geoserverWorkspace)
                ? name
  ...
  addPolygonOverlay(element, layers) {
    ...
    this.probeClick = this.map.on("click", (evt) => {
      const view = this.map.getView(),
        viewResolution = view.getResolution(),
        url1 = this.WMSSource.getFeatureInfoUrl(
          evt.coordinate,
          viewResolution,
          "EPSG:4326",
          {
  ...
  removeLayerByName(name) {
    ...
    const tempNameArr = name.split(":"),
      realName = tempNameArr[tempNameArr.length - 1];
  ...
  zoomToViewByFeature(condition, layerId) {
    ...
    axios.get(wfsUrl).then((res) => {
      if (res.status >= 200 && res.status < 300) {
        ...
        this.map.getView().fit(vecSource.getExtent(), {
          size: this.map.getSize(),
          padding: [200, 500, 200, 400],
        });
      }
    });
  }
